...
--[[
-- Debug
if not oldHookAdd then oldHookAdd = hook.Add end
function hook.Add(...)
    local args = {...}
    oldHookAdd(unpack(args))
    print("added", args[1], args[2])
end

if not oldHookRemove then oldHookRemove = hook.Remove end
function hook.Remove(...)
    local args = {...}
    oldHookRemove(unpack(args))
    print("removed", args[1], args[2])
end
--]]
local ConvarCache = {}
--[convar] = {value, func}, 
local function CreateConVar_either(callback, ...)
    local args = {...}
    if CLIENT then
    ...
    local convarvalue = StandardizeConVarValue(GetConVar(convarname):GetString())
    ConvarCache[convarname] = ConvarCache[convarname] or {}
    ConvarCache[convarname][1] = convarvalue
    ConvarCache[convarname][2] = callback
    cvars.AddChangeCallback(convarname, function(_, _, value_new)
        ConvarCache[convarname][1] = value_new
        local func = ConvarCache[convarname][2]
        if func then
            func(value_new)
            return
        end
...
local function GetConVar_Cached(convarname)
    if ConvarCache[convarname][1] then
        --
        return StandardizeConVarValue(ConvarCache[convarname][1])
    end
...
elseif SERVER then
    ...
    local function GetvFlushPoint(tr, ent)
        if GetConVar_Cached("pk_sv_enable_spawnfix") == 0 then return GetLegacyvFlushPoint(tr, ent) end
...
------------------------------------------------------------------]]
if CLIENT then
    CreateClientConVar_Cached(function(value_new)
        if value_new == "1" then
            hook.Add("PlayerBindPress", CurrentFilePath .. "|Suppressgm_spawnBind", HandleBindPresses)
            return
        elseif value_new == 0 and GetConVar_Cached("pk_grabfix") == 0 and GetConVar_Cached("pk_spawndist") == 2048 and GetConVar_Cached("pk_spawnfix") == 0 then
            hook.Remove("PlayerBindPress", CurrentFilePath .. "|Suppressgm_spawnBind")
        end
    end, "pk_grabfix", "0", true, true, "Whether or not to enable grabfix for yourself")

    CreateClientConVar_Cached(function(value_new)
        if value_new == "2048" then
            hook.Add("PlayerBindPress", CurrentFilePath .. "|Suppressgm_spawnBind", HandleBindPresses)
        elseif value_new ~= "2048" and GetConVar_Cached("pk_grabfix") == 0 and GetConVar_Cached("pk_spawnfix") == 0 then
            hook.Remove("PlayerBindPress", CurrentFilePath .. "|Suppressgm_spawnBind")
        end
    end, "pk_spawndist", "2048", true, true, "Distance to spawn props")

    CreateClientConVar_Cached(function(value_new)
        if value_new == "1" then
            hook.Add("PlayerBindPress", CurrentFilePath .. "|Suppressgm_spawnBind", HandleBindPresses)
            return
        elseif GetConVar_Cached("pk_grabfix") == 0 and GetConVar_Cached("pk_spawndist") == 2048 then
            hook.Remove("PlayerBindPress", CurrentFilePath .. "|Suppressgm_spawnBind")
        end
    end, "pk_spawnfix", "0", true, true, "Whether or not to enable spawnfix for yourself")
elseif SERVER then
    CreateConVar_Cached(nil, "pk_sv_spawndist", "0", FCVAR_ARCHIVE, "Whether or not to allow players to set the distance they spawn props", 0)
    CreateConVar_Cached(nil, "pk_sv_maxspawndist", "4096", FCVAR_ARCHIVE, "Max spawn distance for people using pk_spawndist", 0)
    CreateConVar_Cached(function(value_new)
        if value_new == "1" then
            hook.Add("PlayerSpawnedProp", CurrentFilePath .. "|pk_maxsize", RestrictPropDamage)
        elseif value_new == "0" then
            hook.Remove("PlayerSpawnedProp", CurrentFilePath .. "|pk_maxsize")
        end
    end, "pk_sv_enable_maxsize", "0", FCVAR_ARCHIVE, "Whether or not to use value from pk_sv_maxsize", 0)

    CreateConVar_Cached(nil, "pk_sv_maxsize", "136", FCVAR_ARCHIVE, "The maximum bounding radius (center to furthest corner) a prop can be to do damage. Used to restrict overly large props (8x8 cubes) from propkill. \nREFERENCE: tide = 136, frige = 49, moped = 28", 0)
